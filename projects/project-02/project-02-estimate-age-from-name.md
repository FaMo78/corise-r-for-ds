...
tbl_lifetables <- read_csv(file_names, show_col_types = FALSE)
...
tbl_lifetables |> 
  ...
  ggplot(aes(x = year, y = lx/1e5, color = sex)) +
  # Add a step layer
  geom_step() +
  # Add labels (title, subtitle, x, y, caption)
  labs(
    title = "Probability of Survival of babies which were born in different years",
    subtitle = "Seperate for each Sex (Based On Deacdes Data)",
    x = "Birth Year",
    y = " Probability of Survival",
    caption = "Source: SSA Website"
  ) +
...
tbl_lifetables_extended_2022 |> 
  ...
  labs(
    title = "Probability of Survival of babies which were born in different years",
    subtitle = "Seperate for each Sex (Based On Interpolated data for each year)",
    x="Year",
    y="Probability of Survival",
    caption="Source: SSA Website"
  )+
...
plot_estimated_age <- function(tbl, my_name, my_sex) {
  tbl |> 
    ...
    geom_col(aes(y=nb_alive), color="blue", alpha=0.3)+
    ...
    labs(
      ...
      subtitle = "___",
      x = "Birth Year",
      y = "Number of births/Number of people Alive"
    ) +
...
tbl_names_female_top_25 <- tbl_names |>
  ...
  summarize(total_nb_births=sum(nb_births), .groups="drop") |>
  # Remove column nb_births
  # by summarizing the table it is already dropped
  # Filter only Females
  filter(sex=="F") |>
  # Slice top 25 rows
  arrange(desc(total_nb_births)) |> slice_head(n=25) |>
  # Select name and sex
  select(name, sex)
...
tbl_names_extended_age <- tbl_names_extended |> 
  ...
  ungroup() |> arrange(desc(age_median)) |>
  # Clean the names
  janitor::clean_names()
